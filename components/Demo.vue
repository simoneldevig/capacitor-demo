...
<template>
  <div class="relative flex items-top justify-center min-h-screen bg-gray-100 sm:items-center sm:pt-0">
    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 pt-5">
      ...
      <div class="mt-8 bg-white overflow-hidden shadow sm:rounded-lg p-6 flex justify-center flex-col">
        <h2 class="text-2xl leading-7 font-semibold text-center mb-4">
          Welcome to your Capacitor Nuxt demo application
        </h2>

        <button class="bg-blue-500 hover:bg-blue-700 text-white py-2 px-4 mt-5 rounded" @click="openActionSheet">
          Open action sheet
        </button>

        <button class="bg-green-500 hover:bg-green-700 text-white py-2 px-4 mt-5 rounded" @click="openInAppBrowser">
          Open in-app browser
        </button>

        <button class="bg-red-500 hover:bg-red-700 text-white py-2 px-4 mt-5 rounded" @click="setStatusbarStyle">
          Change statusbar style
        </button>

        <button class="bg-indigo-500 hover:bg-indigo-700 text-white py-2 px-4 mt-5 rounded" @click="showToastMessage">
          Show toast message
        </button>

        <h4 class="font-bold mt-5" v-if="result">Chosen action sheet option: {{ result.index }}</h4>
      </div>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      async openInAppBrowser() {
        await Browser.open({ url: 'https://www.meetup.com/vue-js-aarhus', presentationStyle: 'popover' });
      },
      async setStatusbarStyle() {
        const statusbarInfo = await StatusBar.getInfo();
        if (statusbarInfo.style === 'LIGHT') {
